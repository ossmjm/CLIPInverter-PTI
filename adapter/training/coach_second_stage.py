...
from adapter.configs import data_configs
from adapter.datasets.images_text_dataset import ImagesTextDataset
...
class Coach:
	def __init__(self, opts):
		...
		# Initialize network
		self.net = CLIPAdapterWithDecoder(self.opts).to(self.device)
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				w_hat_clone = w_hat.clone().detach().requires_grad_(True)
				with torch.no_grad():
					y_hat_fixed, _ = self.net.decoder([w_hat], input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1)
				y_hat, w_hat = self.net.decoder([w_hat], input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1, txt_embed=txt_embed_mismatch)
				y_hat = self.face_pool(y_hat)
				y_hat_fixed = self.face_pool(y_hat_fixed)
				loss, loss_dict, id_logs = self.calc_loss(x, y, y_hat, text_original, text_mismatch, w_hat, y, mismatch_text, y_hat_fixed, data_type="real")
				loss.backward()
				...
				w_hat_cycle_clone = w_hat_cycle.clone().detach().requires_grad_(True)
				with torch.no_grad():
					y_recovered_fixed, _ = self.net.decoder([w_hat_cycle], input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1)
				y_recovered, w_hat_cycle = self.net.decoder([w_hat_cycle], input_is_latent=True, return_latents=True, randomize_noise=False, truncation=1, txt_embed=txt_embed_clone)
				y_recovered = self.face_pool(y_recovered)
				y_recovered_fixed = self.face_pool(y_recovered_fixed)
				loss, cycle_loss_dict, cycle_id_logs = self.calc_loss(x, y, y_recovered, text_mismatch, text_clone, w_hat_cycle, y_hat_clone, mismatch_text, y_recovered_fixed, data_type="cycle")
				loss.backward()
				self.optimizer.step()
				...
				if self.global_step % self.opts.image_interval == 0 or \
					...
					self.parse_and_log_images(id_logs, x, y_hat_fixed, y_hat, y_recovered, txt, mismatch_text,
	...
	def validate(self):
		...
		for batch_idx, batch in enumerate(self.test_dataloader):
			...
			with torch.no_grad():
				...
				y_hat = self.face_pool(y_hat)
				y_hat_fixed = self.face_pool(y_hat_fixed)
				loss, cur_loss_dict, id_logs = self.calc_loss(x, y, y_hat, text_original, text_mismatch, w_hat, y, mismatch_text, y_hat_fixed)
			...
			# Logging related
			self.parse_and_log_images(id_logs, x, y_hat_fixed, y_hat, y_recovered, txt, mismatch_text, title='images/test/faces',
	...
	def configure_optimizers(self):
		...
		self.requires_grad(self.net.adapter, False)
		self.requires_grad(self.net.decoder.affine_mlps, True)
		self.requires_grad(self.net.decoder.lambdas, True)
		params = list(self.net.decoder.affine_mlps.parameters())
		params += list(self.net.decoder.lambdas.parameters())
	...
	def calc_loss(self, x, y, y_hat, source_text, target_text, latent, directional_source, mismatch_text, y_hat_fixed, data_type="real"):
		loss_dict = {}
		id_logs = []
		loss = 0.0
		...
		if self.opts.l2_lambda > 0:
			loss_l2 = F.mse_loss(y_hat, y_hat_fixed) * 0.35
			loss_l2 += F.mse_loss(y_hat, y)
			loss_dict[f'loss_l2_{data_type}'] = float(loss_l2)
			loss += loss_l2 * self.opts.l2_lambda
            
		if self.opts.lpips_lambda > 0:
			loss_lpips = self.lpips_loss(y_hat, y_hat_fixed) * 0.35
	...
	def parse_and_log_images(self, id_logs, x, y, y_hat, y_recovered, txt, mismatch_text, title, subscript=None, display_count=2):
		...
		for i in range(display_count):
			cur_im_data = {
				...
				'frozen_face': common.tensor2im(y[i]),
	...
	def log_images(self, name, txt, mismatch_text, im_data, subscript=None, log_latest=False):
		fig = common.vis_faces_frozen(im_data, txt, mismatch_text)
